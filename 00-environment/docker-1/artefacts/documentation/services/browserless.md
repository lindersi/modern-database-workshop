...
Navigate to <http://192.168.1.63:28423> for the built-in UI (API playground and debugger).

### REST API examples

**Take a screenshot:**
```bash
curl -X POST http://192.168.1.63:28423/screenshot \
...
**Generate a PDF:**
```bash
curl -X POST http://192.168.1.63:28423/pdf \
  -H 'Content-Type: application/json' \
  -d '{"url": "https://example.com"}' \
  --output page.pdf
```
...
const browser = await puppeteer.connect({
  browserWSEndpoint: 'ws://192.168.1.63:28423'
});
```

When a token is set, append it as a query parameter:
```
ws://192.168.1.63:28423?token=my-secret-token
```
